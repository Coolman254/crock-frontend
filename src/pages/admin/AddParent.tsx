...
import { parentCrudApi, studentCrudApi } from "@/lib/api";
import { useToast } from "@/hooks/use-toast";
import { useNavigate } from "react-router-dom";
import { X, Search, UserPlus, GraduationCap, CheckCircle2, Loader2, Info } from "lucide-react";
...
export default function AddParentPage() {
  ...
  const [form, setForm] = useState({
    ...
    relationship: "Father", notificationMethod: "app",
  });
  ...
  const handleCreate = async () => {
    if (!form.firstName || !form.lastName || !form.gender || !form.age || !form.nationalId || !form.relationship) {
    ...
    setLoading(true);
    try {
      // Single call — no auth account needed, parents are record-only
      ...
      setDone(true);
      toast({ title: "Parent registered!", description: `${form.firstName} ${form.lastName} added successfully.` });
    } catch (e: any) {
      toast({ title: "Failed to register parent", description: e.message, variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const resetForm = () => {
    setForm({ firstName: "", lastName: "", gender: "Male", age: "", email: "",
      phone: "", nationalId: "", relationship: "Father", notificationMethod: "app" });
    setSelectedStudents([]);
    setStudentSearch("");
    setDone(false);
  };
    ...
    <AdminLayout title="Add Parent">
      <Card className="max-w-lg">
        <CardContent className="py-12 flex flex-col items-center gap-4 text-center">
          ...
          <div>
            <p className="font-semibold text-green-700 dark:text-green-400 text-lg">Parent Registered!</p>
            <p className="text-sm text-muted-foreground mt-1">{form.firstName} {form.lastName} has been added.</p>
          </div>
      ...
      <Card className="max-w-lg">
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            <UserPlus className="h-4 w-4" /> New Parent
          </CardTitle>
          <CardDescription>Register a parent and link them to their child(ren).</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><Label>Email</Label><Input type="email" value={form.email} onChange={e => f("email", e.target.value)} placeholder="parent@example.com" /></div>
          </div>
                  ...
                  {studentsLoading ? (
                    ...
                    filteredStudents.slice(0, 10).map(s => (
                      <button key={s._id} type="button"
                        ...
                        <span className="font-medium">{s.firstName} {s.lastName}</span>
                        <span className="text-muted-foreground text-xs ml-2">#{s.admissionNo} · {s.class}</span>
                      </button>
                    ))
                  )}
                </div>
          ...
          <Button className="w-full h-11 font-semibold mt-2" onClick={handleCreate} disabled={loading}>
            {loading
              ...
              : <><UserPlus className="h-4 w-4 mr-2" />Register Parent</>
            }
          </Button>
